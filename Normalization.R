...
#check the functional depedencies:
...
check_dependencies <- function(table, columns_to_check, num, key) {
  ...
  for(col in names(columns_to_check)){
    ...
    if( all(dependency_check== TRUE) ){
      lista_100 <-c(lista_100, c(col, 100))
    } 
    if(sum(dependency_check)/length(dependency_check) >=num & sum(dependency_check)/length(dependency_check) < 1 ){
      lista_num <- c(lista_num, c(col,sum(dependency_check)/length(dependency_check)*100))
    }
  }
...
check_dependencies_NA <- function(table, columns_to_check, num, key) {
  ...
  for(col in names(columns_to_check)){
    ...
    if( all(dependency_check== TRUE) ){
      lista_100_NA <-c(lista_100_NA, c(col, 100))
    } 
    if(sum(dependency_check)/length(dependency_check) >=num & sum(dependency_check)/length(dependency_check) < 1 ){
      lista_num_NA <- c(lista_num_NA, c(col,sum(dependency_check)/length(dependency_check)*100))
    }
  }
...
check <- function(table, columns_to_check, num, key,con_o_senze_na){
  if(con_o_senze_na){
...
key <- 'NDG'
num <- 0.8
columns_to_check <- Loans_table %>% select(-key,-column_to_eliminate)

x <- check(Loans_table,columns_to_check, num, key,TRUE)
...
print_lists <- function(lists,num){
  for(j in 1:length(x)){
    ...
    for (i in uneven_indices) {
      value <- as.numeric(x[[j]][[i+1]])
      formatted_ratio <- paste0(format(round(value, 2), nsmall = 2), "%")
      cat("Column Name: ", x[[j]][[i]], "  Ratio: ", formatted_ratio, "\n")
    }
    cat("\n")
  }
}
